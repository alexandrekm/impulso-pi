...
export const FEATURES: Feature[] = [
  ...
  },
  {
    ...
    description:
      "Block zvec_index calls against roots that must not be indexed: $HOME (a home index makes every zg call stat the whole home tree — hangs for minutes) and umbrella roots (≥3 nested git repos, which zg cannot index; the stub only shadows real leaf indexes). allowRoots in pi-zvec-grep/config.json is the escape hatch; drop stays allowed. Mirrors the fork's own policy.",
    kind: "local",
  },
  {
    id: "zvec-autoindex",
  ...
  // ── Modes ──────────────────────────────────────────────────────────────
  {
    ...
    description:
      "Per-profile mode that gates which skills reach the system prompt. /mode code|doc|toggle; code = full dev workflow (jira, commit), doc = docs authoring/publishing (gws-docs-authoring, unslop, confluence) + glean + datadog + gws skills (Docs/Sheets/Drive/Gmail, injected in doc mode). /create-pr is a prompt template, unaffected by modes. Per-session (in-memory), applies next message. Work-only.",
    kind: "local",
  },
  // ── Orca & Misc ────────────────────────────────────────────────────────
  {
