...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("turn_end", async (event, ctx) => {
    ...
    const cacheWrite = completedMessage.usage.cacheWrite ?? 0;
    const safeCacheWrite = Number.isFinite(cacheWrite) ? Math.max(0, cacheWrite) : 0;
    state.totalCacheWrite += safeCacheWrite;
    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      function readCurrentMode(): string | null {
        ...
        try {
          ...
          // modes extension not installed on this profile
        }
        return null;
      }
      ...
      // GitHub PR status for the current branch (gh pr view). Refreshed on
      // branch change + on a slow cadence: 20s when a PR exists, 5min when not.
      // Requires the gh CLI; inert (null) when gh is missing or no PR is found.
      let prStatus: string | null = null;
      let ciStatus: string | null = null;
      let lastPr = "";
      let prRefreshInFlight = false;
      let prTimer: ReturnType<typeof setTimeout> | null = null;

      function schedulePr(ms: number): void {
        if (prTimer) clearTimeout(prTimer);
        prTimer = setTimeout(() => void refreshPr(), ms);
        if (typeof (prTimer as any).unref === "function") (prTimer as any).unref();
      }

      async function refreshPr(): Promise<void> {
        if (prRefreshInFlight) return;
        prRefreshInFlight = true;
        try {
          ...
          if (result.code === 0 && result.stdout) {
            try {
              ...
              prStatus = `#${data.number} ${status}`;
              // Fetch CI check summary for this PR's head ref. Runs only when a
              // PR exists; best-effort — failures leave ciStatus null (→ "CI --").
              try {
                const checks = await pi.exec(
                  "gh",
                  ["pr", "checks", "--json", "bucket"],
                  { cwd: ctx.cwd },
                );
                if (checks.code === 0 && checks.stdout) {
                  const rows = JSON.parse(checks.stdout) as Array<{ bucket: string }>;
                  const failed = rows.filter((r) => r.bucket === "fail").length;
                  const pending = rows.filter((r) => r.bucket === "pending").length;
                  if (rows.length === 0) ciStatus = null;
                  else if (failed > 0) ciStatus = `❌${failed}`;
                  else if (pending > 0) ciStatus = "⏳";
                  else ciStatus = "✅";
                } else {
                  ciStatus = null;
                }
          ...
          prRefreshInFlight = false;
          if ((prStatus ?? "") !== lastPr) {
            lastPr = prStatus ?? "";
            tui.requestRender();
          }
          schedulePr(prStatus ? 60_000 : 5 * 60_000);
        }
      }
