import type { PresetName, SegmentKey, SegmentMetadata, SettingsConfig } from "./types.js";

export const DEFAULT_SETTINGS: SettingsConfig = {
  version: 1,
  preset: "standard",
  segments: {
    modelThink: true,
    runtime: true,
    pwd: true,
    git: true,
    mode: true,
    contextUsage: true,
    contextProgress: true,
    contextPercentage: true,
    contextNumbers: true,
    tokens: true,
    tps: true,
    cost: true,
    cache: true,
    cacheWrite: false,
    cacheTtl: true,
    reasoning: true,
    prStatus: true,
    ciStatus: true,
    turnCount: false,
    usageBars: true,
  },
  contextZones: { expert: 70, warning: 85 },
};

export const PRESETS: Record<PresetName, Partial<Record<SegmentKey, boolean>>> = {
  minimal: {
    modelThink: true,
    contextUsage: true,
    contextProgress: true,
    contextPercentage: false,
    contextNumbers: true,
    runtime: false,
    pwd: false,
    git: false,
    mode: true,
    tokens: false,
    tps: false,
    cost: false,
    cache: false,
    cacheWrite: false,
    cacheTtl: false,
    reasoning: false,
    prStatus: false,
    ciStatus: false,
    turnCount: false,
    usageBars: true,
  },
  standard: {
    modelThink: true,
    runtime: true,
    pwd: true,
    git: true,
    mode: true,
    contextUsage: true,
    contextProgress: true,
    contextPercentage: true,
    contextNumbers: true,
    tokens: true,
    tps: false,
    cost: true,
    cache: true,
    cacheWrite: false,
    cacheTtl: true,
    reasoning: true,
    prStatus: true,
    ciStatus: true,
    turnCount: false,
    usageBars: true,
  },
  verbose: {
    modelThink: true,
    runtime: true,
    pwd: true,
    git: true,
    mode: true,
    contextUsage: true,
    contextProgress: true,
    contextPercentage: true,
    contextNumbers: true,
    tokens: true,
    tps: true,
    cost: true,
    cache: true,
    cacheWrite: true,
    cacheTtl: true,
    reasoning: true,
    prStatus: true,
    ciStatus: true,
    turnCount: true,
    usageBars: true,
  },
  performance: {
    modelThink: true,
    runtime: false,
    pwd: false,
    git: false,
    mode: false,
    contextUsage: true,
    contextProgress: false,
    contextPercentage: true,
    contextNumbers: true,
    tokens: false,
    tps: true,
    cost: true,
    cache: false,
    cacheWrite: false,
    cacheTtl: false,
    reasoning: false,
    prStatus: false,
    ciStatus: false,
    turnCount: false,
    usageBars: false,
  },
};

export const SEGMENT_METADATA: SegmentMetadata[] = [
  {
    id: "modelThink",
    label: "Model & Thinking",
    description: "Show current model and thinking level",
  },
  { id: "runtime", label: "Runtime", description: "Show session runtime timer" },
  { id: "pwd", label: "Working Directory", description: "Show current working directory" },
  { id: "git", label: "Git Branch & Diff", description: "Show git branch and diff stats" },
  { id: "mode", label: "Mode", description: "Show current /mode (from the modes extension)" },
  {
    id: "contextUsage",
    label: "Context Usage",
    description: "Master toggle for the context usage segment",
  },
  {
    id: "contextProgress",
    label: "  └ Progress Bar",
    description: "Show the progress bar in context usage",
  },
  {
    id: "contextPercentage",
    label: "  └ Percentage",
    description: "Show the percentage in context usage",
  },
  {
    id: "contextNumbers",
    label: "  └ Used / Total",
    description: "Show the token count in context usage",
  },
  { id: "tokens", label: "Session Tokens", description: "Show total input/output token counts" },
  { id: "tps", label: "Generation Speed", description: "Show live updates/sec and completed-turn tokens/sec" },
  { id: "cost", label: "Cost", description: "Show estimated session cost" },
  { id: "cache", label: "Cache Hit Rate", description: "Show prompt cache efficiency" },
  { id: "cacheWrite", label: "Cache Write Tokens", description: "Show total cache-write tokens" },
  {
    id: "cacheTtl",
    label: "Cache TTL",
    description: "Show prompt-cache retention (short/long) and countdown to expiry (Anthropic semantics: 5m / 1h)",
  },
  { id: "reasoning", label: "Reasoning Tokens", description: "Show total reasoning/thinking tokens" },
  { id: "prStatus", label: "GitHub PR Status", description: "Show current branch PR status via gh" },
  { id: "ciStatus", label: "CI Check Status", description: "Show PR CI checks summary via gh (✅ / ❌N / ⏳)" },
  { id: "turnCount", label: "Turn Counter", description: "Show current turn number" },
  { id: "usageBars", label: "Quota Usage Bars", description: "Show provider subscription usage windows" },
];

export const ZONE_VALUE_OPTIONS = {
  expert: ["60", "65", "70", "75", "80"],
  warning: ["75", "80", "85", "90", "95"],
};
