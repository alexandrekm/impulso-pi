...
Load this file when something in the main flow doesn't go as expected. Rule details live in `skill://create-pr/REFERENCE.md`.
...
| "Commit format is close enough" | Close enough will be blocked by commitlint. Match the regex exactly (see REFERENCE.md). |
...
| "Assume the reference rules apply as-is" | Check the repo's own `commitlint.config.js` first — follow it if it differs. |
