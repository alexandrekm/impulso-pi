...
```bash
# Nothing to export — all three come from the environment. Verify before use:
: "${ATLASSIAN_SITE:?unset}" "${ATLASSIAN_EMAIL:?unset}" "${ATLASSIAN_API_KEY:?unset}"
```

All requests use Basic Auth against `$ATLASSIAN_SITE`. Use `/rest/api/2/` (not v3) for plain-text `description` fields — v3 requires ADF.

## Search (JQL)

```bash
curl -s "${ATLASSIAN_SITE}/rest/api/2/search?jql=<URL_ENCODED_JQL>&maxResults=20" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | python3 -c "import json,sys; [print(f'{i[\"key\"]:15s} {i[\"fields\"][\"issuetype\"][\"name\"]:10s} {i[\"fields\"][\"status\"][\"name\"]:15s} {i[\"fields\"][\"summary\"][:60]}') for i in json.load(sys.stdin).get('issues',[])]"
```

## View ticket

```bash
curl -s "${ATLASSIAN_SITE}/rest/api/2/issue/<KEY>?fields=key,issuetype,summary,status,priority,assignee,reporter,description" \
...
```bash
curl -s -X POST "${ATLASSIAN_SITE}/rest/api/2/issue" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" \
...
Get your account ID: `curl -s "${ATLASSIAN_SITE}/rest/api/2/myself" -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | python3 -c "import json,sys; print(json.load(sys.stdin)['accountId'])"`
...
curl -s "${ATLASSIAN_SITE}/rest/api/2/issue/<KEY>/transitions" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | \
...
curl -s -X POST "${ATLASSIAN_SITE}/rest/api/2/issue/<KEY>/transitions" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" -H "Content-Type: application/json" \
  -d '{"transition":{"id":"<ID>"}}'
```
...
```bash
# 1. Find board ID
BOARD_ID=$(curl -s "${ATLASSIAN_SITE}/rest/agile/1.0/board?projectKeyOrId=<PROJECT_KEY>&type=scrum" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | python3 -c "import json,sys; print(json.load(sys.stdin)['values'][0]['id'])")

# 2. Get active sprint ID
SPRINT_ID=$(curl -s "${ATLASSIAN_SITE}/rest/agile/1.0/board/${BOARD_ID}/sprint?state=active" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | python3 -c "import json,sys; print(json.load(sys.stdin)['values'][0]['id'])")

# 3. Add issue to sprint
curl -s -X POST "${ATLASSIAN_SITE}/rest/agile/1.0/sprint/${SPRINT_ID}/issue" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" \
  -H "Content-Type: application/json" \
  -d "{\"issues\": [\"<JIRA_KEY>\"]}"
```
...
`acli` has no `--story-points` flag. `customfield_10004` = Story Points field on this Jira site ($ATLASSIAN_SITE).
...
```bash
curl -s "${ATLASSIAN_SITE}/rest/api/2/issue/createmeta?projectKeys=<PROJECT_KEY>&issuetypeNames=Epic&expand=projects.issuetypes.fields" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | \
...
```bash
curl -s "${ATLASSIAN_SITE}/rest/api/2/project" \
  -u "${ATLASSIAN_EMAIL}:${ATLASSIAN_API_KEY}" | python3 -c "import json,sys; [print(f'{p[\"key\"]:15s} {p[\"name\"]}') for p in json.load(sys.stdin)]"
```
