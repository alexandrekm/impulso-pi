...
async function handleApi(url: URL, res: http.ServerResponse): Promise<void> {
	...
	if (pathname === "/api/stats/errors") {
		const limit = url.searchParams.get("limit");
		return sendJson(res, 200, await getRecentErrors(range, limit ? parseInt(limit, 10) : undefined));
	}
...
export function startServer(port = 3847, host = "127.0.0.1"): Promise<{ hostname: string; port: number; stop: () => void }> {
