...
| Approval mode (tool gating) | `@gotgenes/pi-permission-system` | ✅ partial (pi-permission-system: granular allow/ask/deny per tool/bash/path with UI confirmation; omp: built-in always-ask/write/yolo modes with per-tool overrides, safety overrides for critical patterns, ACP support) |
