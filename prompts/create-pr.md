...
If anything goes wrong (pre-commit failures, diverged push, non-conforming commits): `read skills/create-pr/TROUBLESHOOTING.md` (path relative to the pi config dir).

Announce at start: "I'm using the /create-pr command to commit and create your PR."
...
No key extractable → load the `jira` skill (`read skills/jira/SKILL.md`) and follow section B (Attach a Jira ticket) to find or create a ticket and return `JIRA_KEY`. This ensures every commit traces to a Jira ticket — no exceptions.
...
Pre-commit hooks fire automatically. Failure → `read skills/create-pr/TROUBLESHOOTING.md` (path relative to the pi config dir).
...
Check each against the format (after excluding "Initial Plan" commits). Non-conforming commit(s) → `read skills/create-pr/TROUBLESHOOTING.md` (path relative to the pi config dir).
...
Remote branch diverged → `read skills/create-pr/TROUBLESHOOTING.md` (path relative to the pi config dir).
...
This is the `jira` skill's B3.c procedure (`read skills/jira/SKILL.md`) — load it for the full placeholder checklist and the REST fallback. If it deviates only slightly, note the deviation in the update body rather than blocking with a prompt.

Also ensure the rest of B3 is satisfied for the ticket: B3.a (In Progress), B3.b (active sprint), and B3.d (story points — ask the user with suggested Fibonacci values if missing). Read `skills/jira/SKILL.md` for those procedures.
...
If `acli` is unavailable, read `skills/jira/FALLBACK.md` and use the REST comment endpoint (`POST /rest/api/2/issue/<KEY>/comment`). Skip if the ticket already has a comment containing this PR URL — don't duplicate on re-runs. Full procedure: `skills/jira/SKILL.md` section C.

## 13. Report

Output the PR URL and the Jira comment confirmation.

<!-- Vendored from skills/create-pr/SKILL.md as a pi prompt template (user-invocation command /create-pr). skill:// references rewritten to `read` paths relative to the pi config dir. -->
